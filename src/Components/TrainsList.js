...
const TrainItem = (props) => {
  const { trainDetails, setShowLoginModal } = props;
  const { trainName, trainNumber, fromStation, toStation, departureTime, arrivalTime, arrivalDate, departureDate, berths } = trainDetails;
  const [selectedBerth, setSelectedBerth] = useState(null);
  const [showContainer, setShowContainer] = useState(false);
  const [closeClicked, setCloseClicked] = useState(false);
  const [showPrice, setShowPrice] = useState(false);
  const [selectedBerthPrice, setShowBerthPrice] = useState(0);
  const [activeBerthType, setActiveBerthType] = useState(null);
  ...
  const [berthTypeArray, setBerthTypeArray] = useState([]);
  const [bookNowButton, setBookNowButton] = useState(false);
  const handleBerthClick = (berthType) => {
    setSelectedBerth(berthType);
    setShowContainer(true);
    setActiveBerthType(berthType);
    setBerthTypeArray(prev => [...prev, berthType]);
  };
  ...
  const handleFinalBookNowClick = async () => {
    if (!selectedBerth) {
      alert("Please select a berth before booking.");
      return;
    }
    ...
    const berthObj = berths.find(b => b.type === selectedBerth);
    navigate("/passenger-details", { state: { train: trainDetails, berth: berthObj } });
  };
  const handleBookNowButton = (berth) => {
    setBookNowButton(true);
    setShowPrice(true);
    setActiveBerthType(berth);
    setShowBerthPrice(berth.price)
  }
  const getButtonBackground = (berthType) => {
    return activeBerthType === berthType ? 'pink' : '';
  };

  const bgroundColor = bookNowButton ? 'red' : 'pink';
  return (
    <div className='TrainItemContainer'>
      <div className='TrainNameContainer'> <h5 style={{ fontWeight: 'bold' }}>{trainName}({trainNumber})</h5></div>
      <h6 style={{ paddingLeft: '10px', marginTop: '20px', marginBottom: '20px' }}>
        <span style={{ fontWeight: 'bold', paddingRight: '10px' }}>{departureTime}</span>|<span style={{ paddingLeft: '10px' }}>{fromStation}</span> | {DepatureformattedDate} <span className='ms-5 me-5'>
          ----------</span>
        <span style={{ fontWeight: 'bold', paddingRight: '10px' }}>{arrivalTime}</span>|
        <span style={{ paddingLeft: '10px' }}>{toStation}</span>|{ArrivalformattedDate}</h6>
      <div className={`${showContainer ? 'berthSelectedContainer' : 'BerthContainer'}`}>

        {!showContainer && closeClicked ? (
          berths.map((eachberth) =>
            berthTypeArray.includes(eachberth.type) ? (
              <div key={eachberth.type} style={{ marginBottom: '10px' }}>
                <button
                  className="BerthStyle"
                  onClick={() => handleBookNowButton(eachberth)}
                  style={{ backgroundColor: getButtonBackground(eachberth) }}
                >
                  <span style={{ fontWeight: 'bold' }}>{eachberth.type}</span>
                  <span style={{ color: 'green', fontWeight: 'bold' }}>AVAILABLE  - {eachberth.available}</span>
                  <span>
                    {eachberth.availability}
                  </span>
              ...
              <div key={eachberth.type} style={{ marginBottom: '10px' }}>
                <button
                  className="BerthStyle"
                  onClick={() => handleBerthClick(eachberth.type)}
                >
                  <span style={{ fontWeight: 'bold' }}>{eachberth.type}</span>
                  <span>
                    Refresh
          ...
          (!showContainer && berths.map((eachberth) => (
            <div key={eachberth.type} style={{ marginBottom: '10px' }}>
              <button
                className="BerthStyle"
                onClick={() => {
                  handleBerthClick(eachberth.type)
                  handleBookNowButton(eachberth)
                }}
              >
                <span style={{ fontWeight: 'bold' }}>{eachberth.type}</span>

                <span>
                  Refresh <FontAwesomeIcon style={{ color: 'black' }} icon={faRotateRight} />
                </span>
        ...
        {showContainer && (
          <div className="berthSelectedContainer">
            <div className='ContainerBerthButtonContainer'>
              <div className='BerthsShowConatiner'>
                {berths.map((eachberth) => (
                  <button onClick={() => {
                    handleBerthClick(eachberth.type);
                    handleBookNowButton(eachberth);
                  }}
                    className='ContainerBerthButton' >{eachberth.type}</button>
                ))}</div>
              <button className="closeButton" onClick={handleCloseContainer}>✖</button>
            </div>

            {berths
              .filter(b => b.type === selectedBerth)
              .map(b => (
                <button key={b.type} className="AvailBerthStyle" onClick={() => { handleBookNowButton(b) }} style={{ backgroundColor: getButtonBackground(b) }}>
                  <span style={{ fontWeight: 'bold' }}>{DepatureformattedDate}</span>
                  <span style={{ color: 'green', fontWeight: 'bold' }}>AVAILABLE  - {b.available}</span>
                </button>
              ))}
          </div>
        )}
      ...
      <div className='d-flex flex-row'><button className='BookNowButton' style={{ backgroundColor: bgroundColor }} onClick={handleFinalBookNowClick}>Book Now</button>
        {showPrice && !closeClicked && <p style={{ fontWeight: 'bold', textAlign: 'center', paddingTop: '10px', paddingLeft: '5px' }}>₹ {selectedBerthPrice}</p>}</div>
    </div>
  )
}
...
function TrainList() {
  ...
  useEffect(() => {
    if (fromStation && toStation) {
      fetch(`http://localhost:5000/api/trains?fromStation=${fromStation}&toStation=${toStation}&
departureDate=${departureDate}`)
  ...
  return (
    <>
      ...
      {loading ?
        (
          <div className="d-flex flex-column align-items-center mt-4" style={{ marginTop: '150px' }}>
            <div className="spinner-border text-primary" role="status"></div>
            <p className="mt-2">Fetching trains, please wait...</p>
          </div>
        ) : (<div style={{ marginTop: '130px' }}>
          <h2 style={{ marginLeft: '20px' }}>Trains from {fromStation} to {toStation}</h2>
          {trains.length === 0 ? (
            <p>No trains available.</p>
          ) : (
            <ul>
              {trains.map((train, idx) => (
                <TrainItem trainDetails={train} key={idx} setShowLoginModal={setShowLoginModal} />
              ))}
            </ul>
          )}
          <LoginModal show={showLoginModal} onClose={() => setShowLoginModal(false)} setUser={setUser} />
        </div>
        )}
